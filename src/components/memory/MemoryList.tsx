...
export default function MemoryList({ items, onDelete }: MemoryListProps) {
  if (!items.length) {
    return <p className="px-4 py-5 text-sm text-white/45">No chat memory saved yet.</p>;
  }
      ...
      {items.map((item) => (
        <div key={item.id} className="group flex items-center justify-between gap-4 px-4 py-3">
          <div className="min-w-0 space-y-1">
            <p className="text-[11px] uppercase tracking-[0.16em] text-white/35">{item.role}</p>
